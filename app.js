...
let veryOldInventors = inventors.filter( (person) => {
    return person.year < 1600 && person.year> 1499
})
...
let inventorNames = inventors.map((person) => ({
    first: person.first, 
    last: person.last
}));
...
let commentById = comments.find((com) => {
...
let idx = comments.findIndex((comId) => {
  return comId.id === 123523
});
